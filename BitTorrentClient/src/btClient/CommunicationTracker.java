...
public class CommunicationTracker {
	...
	public static ArrayList<Peer> peersList;
	...
	public void establishConnection() {
		...
		String fullUrl = torrentInfoRU.announce_url.toString() + "?info_hash="
				+ bytesToHex(torrentInfoRU.info_hash.array()) + "&peer_id="
				+ bytesToHex(random_peer) + "&port=" + 6681 + "&uploaded="
		...
		try {
			...
			incomplete = (int) responseMap.get(ByteBuffer.wrap(new byte[] {
					'i', 'n', 'c', 'o', 'm', 'p', 'l', 'e', 't', 'e' }));
			ByteBuffer peer_id = (ByteBuffer) responseMap.get(ByteBuffer
					.wrap(new byte[] { 'p', 'e', 'e', 'r', '_', 'i', 'd' }));
			...
			for (Map<ByteBuffer, Object> temp : peers) {
				...
				/*
				 * (int interval, int complete, int incomplete, String IP,
			...
			/*
			 * peer_bytes=(ByteBuffer) responseMap.get(ByteBuffer.wrap(new
			 * byte[] {'p', 'e','e','r','s'})); String peerID=new
			 * String(peer_bytes.array(), "ASCII"); /*BufferedReader read=new
			 * BufferedReader(new
			 * InputStreamReader(connection.getInputStream())); StringBuffer
			 * response = new StringBuffer();
			 * while(((line=read.readLine())!=null)){ response.append(line); }
			 * read.close();
			 */
