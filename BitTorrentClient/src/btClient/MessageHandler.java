...
public class MessageHandler implements Runnable {
	...
	public MessageHandler(ArrayList<Piece> pieces, Peer peer,
			ByteBuffer clientID, TorrentInfo torr, ArrayList<Peer> peers) {
		this.pieces = pieces;
		this.peer = peer;
		this.info_hash = torr.info_hash;
		...
		while (peer.isConnected()) {
			...
			while (!choked) {
				...
				if (piece != null) {
					...
					sendMessages message = new sendBlock(block,
	...
	private Piece getNextPiece() {
		...
		System.out.println("I have this piece"
				+ peer.getPeerBool()[indices.get(n)]);
		System.out.println("Does the host have it? "
				+ has_piece[indices.get(n)]);
		return piece;
	 ...
	 * @throws BtException
	 */
	private synchronized void handleMessage(byte[] message) throws IOException,
			BtException {
		ByteBuffer parser;
		if (message.length == 0) {
			peer.keepAlive();
		...
		case BtUtils.UNINTERESTED_ID:
			System.out.println("Uninterested Message  "
					+ Thread.currentThread().getId() + " "
					+ Thread.currentThread().getName());
			break;
		case BtUtils.HAVE_ID:
			System.out.println("Have Message " + Thread.currentThread().getId()
					+ " " + Thread.currentThread().getName());
	...
	public void receivePiece(byte [] message) throws IOException, BtException{
		ByteBuffer parser = ByteBuffer.wrap(message);
		int index = parser.getInt(1);
		if(index < 0 || index >= pieces.size()){
			System.err.println(Thread.currentThread().getName() + " Received invalid piece #" + index);
			wasted += message.length;
			return;
		}
		System.out.println(Thread.currentThread().getName() + " received piece " + index);
		...
		if (piece.isComplete()) {
			if (!piece.checkHash(torrent.piece_hashes[piece.getIndex()]
				...
				for (Peer peerFor : this.peers) {
					sendMessages passMess = new sendHave(piece.getIndex(),
							peer.getOutputStream());
